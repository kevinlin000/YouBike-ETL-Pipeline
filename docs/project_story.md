...
9. Next step: rerun full training data evaluation before making model-accuracy claims.

## Open Gaps

| Gap | Why it matters | Best next action |
| --- | --- | --- |
| Full LSTM metrics are not committed | The repo cannot yet prove full out-of-sample model performance. | Run `make train-lstm` where the full processed CSV exists, then preserve `model_metadata.json`. |
