...
5. Push 前執行 `make validate-config`、`make validate-dependencies` 與 `make test`；涉及 dbt 時再執行 `DB_PASSWORD=dummy make dbt-parse`。
...
- Dependency manifest validation：`make validate-dependencies`
